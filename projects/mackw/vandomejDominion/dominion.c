...
int adventurer_effect(struct gameState *state)
{
  int currentPlayer = whoseTurn(state);
  int cardDrawn;
  int drawntreasure = 0;
...
int mine_effect(struct gameState *state, int choice1, int choice2, int handPos)
{
  int i;
  int j;
  int currentPlayer = whoseTurn(state);
...
int smithy_effect(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
  int i;
...
int baron_effect(struct gameState *state, int choice1)
{
  int currentPlayer = whoseTurn(state);
  ...
  {                             //Boolean true or going to discard an estate
    ...
    {
      if (state->hand[currentPlayer][p] <= estate + 1)
...
int ambassador_effect(struct gameState *state, int choice1, int choice2, int handPos)
{
  int currentPlayer = whoseTurn(state);
  int i;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
  case adventurer:
    return adventurer_effect(state);
  ...
  case mine:
    return mine_effect(state, choice1, choice2, handPos);
  case remodel:
    j = state->hand[currentPlayer][choice1]; //store card we will trash
  ...
  case smithy:
    return smithy_effect(state, handPos);
  ...
  case baron:
    return baron_effect(state, choice1);
  ...
  case ambassador:
    return ambassador_effect(state, choice1, choice2, handPos);
